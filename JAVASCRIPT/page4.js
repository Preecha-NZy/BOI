...
function addETC() {
  ...
  if (etc_input.value != "" && !etcData.includes(etc_input.value)) {
    etcData.push(etc_input.value)
    ...
    delete_area.setAttribute('id', `${etc_input.value}`)
    ...
    delete_area.onclick = function () {
      etcData.indexOf(`${delete_area.id}`) !== -1 && etcData.splice(etcData.indexOf(`${delete_area.id}`), 1)
      delete_area.parentElement.remove()
    }
  } else {
    alert("ข้อมูลดังกล่าวได้มีการกรอกแล้ว กรุณาตรวจสอบข้อมูลอีกครั้ง")
    etc_input.value = ""
  }
}
...
function ESS_insert() {
  ...
  if (ess != null) {
    ...
    if (document.getElementById('ESS_Cost').value != '')
      ESS.Cost = parseFloat(document.getElementById('ESS_Cost').value);
    else {
