...
function updatePosition1($doc_no, $assignorName, $conn)
{
    $sql = "update ธุรกรรมล่าสุด set ผู้อำนวยการกอง = ? where doc_no = '" . $doc_no . "'";
    $params = array($assignorName);
    $r_data = sqlsrv_query($conn, $sql, $params);
...
function updatePosition2($doc_no, $assignorName, $conn)
{
...
$assignorName = $_POST['assignorName'];
$assigneeName = $_POST['assigneeName'];
$status1 = $_POST['status'];
$status2 = $_POST['status2'];
$position = $_POST['position'];
insertTransaction($doc_no, $assignorName, $status1, $currentDate, $conn); 
$date2 = date('Y-m-d H:i:s', strtotime('+1 second', strtotime($currentDate)));
insertTransaction($doc_no, $assigneeName, $status2, $date2, $conn);

if ($position == 'ผอกอง') {
    updatePosition1($doc_no, $assignorName, $conn);
} else {
    updatePosition2($doc_no, $assignorName, $conn);
}
